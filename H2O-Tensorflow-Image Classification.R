...
json_file<- "path/train_filename.json"
...
#writing the output to an images file in csv format
  write.csv(output,"path/images_file.csv")

}

data = read.csv("path/images_file.csv")
...
imageClassifier = function(image_link)
{
  ...
  restore$restore(new_session, 'path/vgg_16_file.ckpt')
  ...
  classes = read_delim("path/imagenet_classes_filename.txt", "\t", escape_double = FALSE, trim_ws = TRUE,col_names = FALSE)
...
write.csv(classified_images,"path/image_classification_filename.csv")
